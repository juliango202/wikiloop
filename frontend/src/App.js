...
const WIKIPEDIA_URL_MSG = "Please enter a valid Wikipedia URL, e.g. https://en.wikipedia.org/wiki/Water"
...
class App extends React.Component {
  ...
  handleUrlChange = (event) => {
        ...
        if (!url) {
            this.setState({[event.target.name]: '', [event.target.name + "Error"]: ""})
            return
        }
  ...
  render() {
      ...
      const isValidJourney = state.startUrl && state.stopUrl && !state.startUrlError && !state.stopUrlError
      ...
      return (
        <div className={classes.root}>
          ...
          {gameState !== GameStates.START &&
            <Card className={classes.card}>
              <CardContent>
                ...
                {gameState === GameStates.RESULTS && wikiloop.error &&
          ...
          {gameState === GameStates.RESULTS  && wikiloop.journey &&
